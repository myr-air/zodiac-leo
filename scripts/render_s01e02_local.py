...
def write_subtitles(
    ...
    *,
    ...
    return summary | {
        ...
        "boundary": "subtitle timing source determined by active timing lane; see timing_method for final vs draft status",
    }
...
def build_music_note_overlay(duration: float, temp_root: Path, *, start_seconds: float = 0.0) -> tuple[Path, dict[str, object]]:
    output = temp_root / "s01e02-render-02-music-note-overlay.mov"
    if output.exists() and output.stat().st_size > 0:
        return output, {
            ...
            "note_count": 3,
            "motion": "EP1 render-05 tiny warm vector notes adapted to the S01E02 header zone",
...
def render_header_frame(path: Path, entry: TimelineTrack) -> None:
    ...
    text_soft(image, (224, 34), "MELLOW LONGPLAY  •  S01 - E02", load_font(29), brown)
...
def build_combined_overlay_concat(timeline: list[TimelineTrack], cues: list[Cue], duration: float, temp_root: Path) -> tuple[Path, dict[str, object]]:
    ...
    frame_dir.mkdir(parents=True, exist_ok=True)
    concat_path = temp_root / "s01e02-combined-overlay.ffconcat"
...
def make_particle_texture(path: Path) -> None:
    """Night-classroom dust layer with random-looking drift groups.
...
def build_equalizer_overlay(audio_path: Path, duration: float, temp_root: Path, *, start_seconds: float = 0.0) -> Path:
    output = temp_root / "s01e02-render-02-ribbon-equalizer-overlay.mov"
    if output.exists() and output.stat().st_size > 0:
        return output
...
def build_visual_assets(audio_path: Path, duration: float, temp_root: Path, *, start_seconds: float = 0.0) -> dict[str, Path]:
    particle_path = temp_root / "s01e02-render-02-particles.png"
    star_path = temp_root / "s01e02-render-02-star-light.png"
    lamp_path = temp_root / "s01e02-render-02-lamp-glow.png"
    light_path = temp_root / "s01e02-render-02-light-sweep.png"
    glow_path = temp_root / "s01e02-render-02-night-glow.png"
    reflection_path = temp_root / "s01e02-render-02-reflection.png"
    shadow_path = temp_root / "s01e02-render-02-shadow.png"
...
def build_subtitle_overlay(
    ...
    *,
    ...
    output = temp_root / f"s01e02-subtitle-overlay-{overlay_signature}.mov"
    if output.exists() and output.stat().st_size > 0:
...
def render_video(
    ...
    *,
    ...
    temp_video = temp_root / f"s01e02-local-render-02-{temp_suffix}.tmp.mp4"
...
def run_ffmpeg_segment_render(
    ...
    prepare_output(video_out)
    temp_video = temp_root / f"s01e02-render-02-segment-{segment.index:02d}.tmp.mp4"
    if temp_video.exists():
        temp_video.unlink()
...
def assemble_chunked_video(
    ...
    prepare_output(video_out)
    concat_path = temp_root / "s01e02-render-02-segments.ffconcat"
    write_concat_file(segment_paths, concat_path)
    temp_video = temp_root / "s01e02-render-02-final.tmp.mp4"
    if temp_video.exists():
        temp_video.unlink()
    command = [
        "ffmpeg",
...
def extract_snapshots(video_path: Path, output_root: Path, duration: float) -> list[str]:
    specs = [
        ("s01e02-render-02-sample-01-open.png", 2.0),
        ("s01e02-render-02-sample-02-track03.png", 370.0),
        ("s01e02-render-02-sample-03-track06.png", 900.0),
        ("s01e02-render-02-sample-04-track09.png", 1450.0),
        ("s01e02-render-02-sample-05-track12.png", 2090.0),
        ("s01e02-render-02-sample-06-ending.png", max(1.0, duration - 8.0)),
    ]
...
def main() -> int:
    ...
    summary = {
        ...
        "decode_check": decode,
        "snapshots": snapshots,
        "visual_policy": "EP1 render-05 headphone icon, music notes, particles/light, custom ribbon-dot equalizer, and near-still motion are the canonical video overlay standard; S01E02 adapts them to the night classroom image",
