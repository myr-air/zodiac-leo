#!/usr/bin/env python3
"""Create local S01E05 render/export QA evidence.

This helper is local-only. It reads user-supplied S01E05 audio/image
...
DEFAULT_TEMP_ROOT = Path(tempfile.gettempdir()) / "opencode" / "s01e05-render"
SONG_SOURCE_ROOT = EPISODE_DIR / "source" / "suno-tracks"
BACKGROUND_SOURCE = VISUAL_ROOT / "ChatGPT Image Jun 9, 2026, 04_36_46 PM.png"
SELECTED_BACKGROUND = SELECTED_VISUAL_ROOT / "vis-c01--apartment-window-playlist-cover.png"
...
EQ_WIDTH = 430
EQ_HEIGHT = 140
RENDER_PIPELINE_VERSION = "local-render-01-subtitle-track-alignment-v4"


def pipeline_signature_payload(subtitle_early_offset: float = SUBTITLE_EARLY_OFFSET_SECONDS) -> str:
    return json.dumps(
        {
            "version": RENDER_PIPELINE_VERSION,
            "motion": {
                "particle_motion": "interval_weighted_trig_mix",
                "light_motion": "interval_weighted_trig_mix",
            },
            "filters": {
                "subtitle_fade_alpha": "sigmoid_pow2_ema",
                "subtitle_position": "430x170",
...
def render_manifest_path(output_root: Path) -> Path:
    return output_root / "video" / f".{EPISODE_ID}-local-render-01-manifest.json"
...
class TrackSpec:
    ...
    def selected_raw(self) -> Path:
        return self.raw_a if self.selected_candidate == "c01" else (self.raw_b or self.raw_a)

    @property
    def pool_raw(self) -> "Path | None":
        if self.selected_candidate == "c01":
            return self.raw_b
        return self.raw_a
...
# Flat audio directory: c01 (selected) in audio/ root; c02 (pool) in audio/pool/.
# Track 10 has no pool candidate; raw_b is None.
_AUDIO_C01 = SELECTED_AUDIO_ROOT
_AUDIO_C02 = POOL_AUDIO_ROOT

TRACKS: tuple[TrackSpec, ...] = (
    TrackSpec(1,  "Corner Glow",          _AUDIO_C01 / "aud-t01_c01--corner-glow.wav",          _AUDIO_C02 / "aud-t01_c02--corner-glow.wav"),
    TrackSpec(2,  "Steam Rising",         _AUDIO_C01 / "aud-t02_c01--steam-rising.wav",         _AUDIO_C02 / "aud-t02_c02--steam-rising.wav"),
    TrackSpec(3,  "Reflections on Glass", _AUDIO_C01 / "aud-t03_c01--reflections-on-glass.wav", _AUDIO_C02 / "aud-t03_c02--reflections-on-glass.wav"),
    TrackSpec(4,  "Gravel and Glow",      _AUDIO_C01 / "aud-t04_c01--gravel-and-glow.wav",      _AUDIO_C02 / "aud-t04_c02--gravel-and-glow.wav"),
    TrackSpec(5,  "A Folded Corner",      _AUDIO_C01 / "aud-t05_c01--a-folded-corner.wav",      _AUDIO_C02 / "aud-t05_c02--a-folded-corner.wav"),
    TrackSpec(6,  "Deep Strings",         _AUDIO_C01 / "aud-t06_c01--deep-strings.wav",         _AUDIO_C02 / "aud-t06_c02--deep-strings.wav"),
    TrackSpec(7,  "Kitchen Timer",        _AUDIO_C01 / "aud-t07_c01--kitchen-timer.wav",        _AUDIO_C02 / "aud-t07_c02--kitchen-timer.wav"),
    TrackSpec(8,  "City Bokeh",           _AUDIO_C01 / "aud-t08_c01--city-bokeh.wav",           _AUDIO_C02 / "aud-t08_c02--city-bokeh.wav"),
    TrackSpec(9,  "Shelved Spines",       _AUDIO_C01 / "aud-t09_c01--shelved-spines.wav",       _AUDIO_C02 / "aud-t09_c02--shelved-spines.wav"),
    TrackSpec(10, "Shadows on the Wall",  _AUDIO_C01 / "aud-t10_c01--shadows-on-the-wall.wav",  _AUDIO_C02 / "aud-t10_c02--shadows-on-the-wall.wav"),
    TrackSpec(11, "The Keys by the Door", _AUDIO_C01 / "aud-t11_c01--the-keys-by-the-door.wav", _AUDIO_C02 / "aud-t11_c02--the-keys-by-the-door.wav"),
    TrackSpec(12, "Warm Fabric",          _AUDIO_C01 / "aud-t12_c01--warm-fabric.wav",          _AUDIO_C02 / "aud-t12_c02--warm-fabric.wav"),
    TrackSpec(13, "Winding Down",         _AUDIO_C01 / "aud-t13_c01--winding-down.wav",         _AUDIO_C02 / "aud-t13_c02--winding-down.wav"),
)
...
def ensure_inputs_exist() -> None:
    required: list[Path] = [BACKGROUND_SOURCE, *(track.raw_a for track in TRACKS), *(track.source_pack for track in TRACKS)]
    # Pool file (raw_b) is optional: Track 10 has no c02 variant.
    required += [track.raw_b for track in TRACKS if track.raw_b is not None]
    missing = [str(path) for path in required if not project_path(path).exists()]
    if missing:
        raise FileNotFoundError("missing required S01E05 input(s): " + ", ".join(missing))
...
def organize_candidates() -> dict[str, object]:
    ...
    for track in TRACKS:
        ...
        if track.pool_raw is not None:
            ...
            pool.append({"track": track.number, "candidate_id": track.pool_candidate, "title": track.title, "status": "no_pool_candidate_supplied"})
...
def assign_slots_from_timeline(raw_srt_cues: list[Cue], timeline: list[TimelineTrack]) -> list[Cue]:
    ...
    for raw in raw_srt_cues:
        slot = normalize_slot(raw.start, raw.end, timeline)
        cues.append(Cue(slot, raw.start, raw.end, raw.text))
    return cues


def normalize_slot(start_seconds: float, end_seconds: float, timeline: list[TimelineTrack], *, margin_seconds: float = 0.5) -> str:
    start = max(0.0, min(start_seconds, end_seconds))
    ...
    for entry in timeline:
        ...
        if overlap > best_overlap + 0.0001:
    ...
    if best_slot is not None:
        return f"T{best_slot.spec.number:02d}"

    midpoint = (start + end) / 2.0
    nearest = min(timeline, key=lambda entry: abs((entry.start + entry.end) / 2.0 - midpoint))
    if abs(midpoint - nearest.start) <= margin_seconds or abs(midpoint - nearest.end) <= margin_seconds:
        return f"T{nearest.spec.number:02d}"
    return f"T{nearest.spec.number:02d}"
...
def build_music_note_overlay(duration: float, temp_root: Path, *, start_seconds: float = 0.0) -> tuple[Path, dict[str, object]]:
    output = temp_root / "s01e05-render-01-music-note-overlay.mov"
    if output.exists() and output.stat().st_size > 0:
        return output, {
            ...
            "note_count": 3,
            "motion": "EP1 render-05 tiny warm vector notes adapted to the S01E05 header zone",
...
def particle_motion_expr(gt: str) -> tuple[str, str]:
    ...
    return (
        f"sin({gt}*0.020)*34+cos({gt}*0.026)*30+sin({gt}*0.038+1.1)*22+cos({gt}*0.051+2.8)*17+sin({gt}*0.089+0.4)*9",
        f"-540+sin({gt}*0.015)*36+cos({gt}*0.021)*30+sin({gt}*0.033+1.9)*22+cos({gt}*0.047+0.5)*14+sin({gt}*0.072+1.1)*9",
    )


def light_motion_expr(gt: str) -> tuple[str, str]:
    return (
        f"(sin({gt}*0.022)*58)+(cos({gt}*0.026+0.8)*42)+(sin({gt}*0.047+1.2)*24)+(cos({gt}*0.063+2.4)*14)",
        f"sin({gt}*0.016)*44+cos({gt}*0.023+0.4)*34+sin({gt}*0.038+0.9)*18+cos({gt}*0.054+1.1)*12",
    )
    ...
    payload = json.dumps(
        {
            ...
            "pipeline": pipeline_signature_payload(),
            "duration_seconds": round(duration, 3),
...
def render_header_frame(path: Path, entry: TimelineTrack) -> None:
    ...
    text_soft(image, (224, 34), "MELLOW LONGPLAY  •  S01 - E04", load_font(29), brown)
...
def build_header_overlay_concat(timeline: list[TimelineTrack], duration: float, temp_root: Path) -> tuple[Path, dict[str, object]]:
    ...
    concat_path = temp_root / "s01e05-header-overlay.ffconcat"
...
def build_combined_overlay_concat(timeline: list[TimelineTrack], cues: list[Cue], duration: float, temp_root: Path) -> tuple[Path, dict[str, object]]:
    ...
    The earlier local-render-01 draft tried to pre-render a per-frame subtitle QTRLE
    ...
    frame_dir.mkdir(parents=True, exist_ok=True)
    concat_path = temp_root / "s01e05-combined-overlay.ffconcat"
...
def make_particle_texture(path: Path) -> None:
    """Bookstore dust layer with random-looking drift groups.
...
def build_equalizer_overlay(audio_path: Path, duration: float, temp_root: Path, *, start_seconds: float = 0.0) -> Path:
    output = temp_root / "s01e05-render-01-ribbon-equalizer-overlay.mov"
    if output.exists() and output.stat().st_size > 0:
        return output
...
def build_visual_assets(audio_path: Path, duration: float, temp_root: Path, *, start_seconds: float = 0.0) -> dict[str, Path]:
    particle_path = temp_root / "s01e05-render-01-particles.png"
    star_path = temp_root / "s01e05-render-01-star-light.png"
    lamp_path = temp_root / "s01e05-render-01-lamp-glow.png"
    light_path = temp_root / "s01e05-render-01-light-sweep.png"
    glow_path = temp_root / "s01e05-render-01-night-glow.png"
    reflection_path = temp_root / "s01e05-render-01-reflection.png"
    shadow_path = temp_root / "s01e05-render-01-shadow.png"
...
def build_subtitle_overlay(
    ...
    *,
    ...
    output = temp_root / f"s01e05-subtitle-overlay-{overlay_signature}.mov"
    if output.exists() and output.stat().st_size > 0:
...
def render_video(
    ...
    *,
    ...
    if visual_proof_seconds is None:
        video_name = f"{EPISODE_ID}.local-render-01-draft-subtitled-{subtitle_signature}-1080p24-qa.mp4"
    else:
        safe_seconds = max(1, int(round(visual_proof_seconds)))
        video_name = f"{EPISODE_ID}.local-render-01-visual-proof-{safe_seconds}s-{subtitle_signature}.mp4"
    video_out = output_root / "video" / video_name
    if video_probe_matches(video_out, duration):
    ...
    temp_video = temp_root / f"s01e05-render-01-{temp_suffix}.tmp.mp4"
    ...
    light_x, light_y = light_motion_expr(gt)
    filter_graph = ";".join(
        [
            "[1:a]anull[aout]",
            "[0:v]scale=1928:1085:force_original_aspect_ratio=increase,"
            f"crop=1920:1080:x='(in_w-out_w)/2+sin({gt}*0.022)*0.60+cos({gt}*0.031)*0.30':"
            f"y='(in_h-out_h)/2+cos({gt}*0.018)*0.36+sin({gt}*0.024)*0.28',fps=24,format=rgba[bg]",
            "[9:v]format=rgba[shadow]",
            f"[bg][shadow]overlay=x='sin({gt}*0.034)*3.0+cos({gt}*0.047)*1.4':y='cos({gt}*0.029)*2.0+sin({gt}*0.053)*1.0':format=auto[withshadow]",
            "[3:v]format=rgba[pt]",
            f"[withshadow][pt]overlay=x='{particle_x}':y='{particle_y}':format=auto[withparticles]",
            "[4:v]format=rgba[stars]",
            f"[withparticles][stars]overlay=x='sin({gt}*0.028)*10+cos({gt}*0.049)*6+sin({gt}*0.061+1.4)*2':y='cos({gt}*0.022)*8+sin({gt}*0.041)*3+cos({gt}*0.073+0.8)*1.5':format=auto[withstars]",
            "[5:v]format=rgba[lamp]",
            f"[withstars][lamp]overlay=x='sin({gt}*0.062)*7':y='cos({gt}*0.051)*5+sin({gt}*0.083)*3':format=auto[withlamp]",
            "[6:v]format=rgba[light]",
            f"[withlamp][light]overlay=x='{light_x}':y='{light_y}':format=auto[withlight]",
            "[7:v]format=rgba[glow]",
            f"[withlight][glow]overlay=x='(sin({gt}*0.041)*15)+(cos({gt}*0.062)*5)+(sin({gt}*0.093+2.4)*2)':y='(cos({gt}*0.028)*10)+(sin({gt}*0.047)*4)+(cos({gt}*0.087+1.1)*2)':format=auto[withglow]",
            "[8:v]format=rgba[ref]",
            f"[withglow][ref]overlay=x='sin({gt}*0.044)*12+cos({gt}*0.067)*4':y='cos({gt}*0.049)*6+sin({gt}*0.081)*2':format=auto[withref]",
            "[10:v]format=rgba[eq]",
...
def segment_video_path(output_root: Path, segment: RenderSegment, *, cache_key: str) -> Path:
    return output_root / "video" / "segments" / f"{EPISODE_ID}.local-render-01-segment-{segment.index:02d}-track-{segment.entry.spec.number:02d}-{cache_key}.mp4"
...
def run_ffmpeg_segment_render(
    ...
    segment_signature_payload = (
        f"{cue_signature(local_cues)}|{segment.entry.spec.number:02d}|{segment.index:02d}|"
        f"{round(segment.start,3)}|{round(segment.end,3)}|{round(subtitle_early_offset,3)}|"
        f"{pipeline_signature_payload(subtitle_early_offset)}"
    )
    ...
    if video_out.exists():
        video_out.unlink()
    prepare_output(video_out)
    temp_video = temp_root / f"s01e05-render-01-segment-{segment.index:02d}.tmp.mp4"
    if temp_video.exists():
        temp_video.unlink()
...
def assemble_chunked_video(
    ...
    video_out = output_root / "video" / f"{EPISODE_ID}.local-render-01-draft-subtitled-{video_signature}-1080p24-qa.mp4"
    if video_probe_matches(video_out, duration):
        return video_out, {"reused_existing": True, "segment_count": len(segment_paths)}
    prepare_output(video_out)
    concat_path = temp_root / "s01e05-render-01-segments.ffconcat"
    write_concat_file(segment_paths, concat_path)
    temp_video = temp_root / "s01e05-render-01-final.tmp.mp4"
    if temp_video.exists():
        temp_video.unlink()
    command = [
        "ffmpeg",
...
def render_chunked_video(
    ...
    video_path, assembly_summary = assemble_chunked_video(segment_paths, audio_abs, output_root, temp_root, duration, video_signature)
    return video_path, {
...
def extract_snapshots(video_path: Path, output_root: Path, duration: float) -> list[str]:
    specs = [
        ("s01e05-render-01-sample-01-open.png", 2.0),
        ("s01e05-render-01-sample-02-track03.png", 370.0),
        ("s01e05-render-01-sample-03-track06.png", 900.0),
        ("s01e05-render-01-sample-04-track09.png", 1450.0),
        ("s01e05-render-01-sample-05-track12.png", 2090.0),
        ("s01e05-render-01-sample-06-ending.png", max(1.0, duration - 8.0)),
    ]
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        type=float,
        default=None,
        help="Render a short visual proof with the local-render-01 overlay/motion stack instead of the full longplay.",
    )
    parser.add_argument(
    ...
    summary = {
        "episode_id": EPISODE_ID,
        "status": "local_render_01_visual_revision_created_with_ep1_render05_overlay_motion_standard_needs_human_watch",
        "intake": intake,
        ...
        "decode_check": decode,
        "snapshots": snapshots,
        "visual_policy": "EP1 render-05 headphone icon, music notes, particles/light, custom ribbon-dot equalizer, and near-still motion are the canonical video overlay standard; S01E05 adapts them to the apartment window image",
