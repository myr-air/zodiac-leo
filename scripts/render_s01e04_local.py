#!/usr/bin/env python3
"""Create local S01E04 render/export QA evidence.

This helper is local-only. It reads user-supplied S01E04 audio/image
...
DEFAULT_TEMP_ROOT = Path(tempfile.gettempdir()) / "opencode" / "s01e04-render"
SONG_SOURCE_ROOT = EPISODE_DIR / "source" / "suno-tracks"
BACKGROUND_SOURCE = VISUAL_ROOT / "ChatGPT Image Jun 3, 2026, 01_50_09 PM.png"
SELECTED_BACKGROUND = SELECTED_VISUAL_ROOT / "vis-c01--bookstore-afternoon-playlist-cover.png"
...
_AUDIO_C01 = AUDIO_ROOT
_AUDIO_C02 = POOL_AUDIO_ROOT

TRACKS: tuple[TrackSpec, ...] = (
    TrackSpec(1,  "Quiet Cover",          _AUDIO_C01 / "Quiet Cover.wav",          _AUDIO_C02 / "Quiet Cover.wav"),
    TrackSpec(2,  "Between Two Chapters", _AUDIO_C01 / "Between Two Chapters.wav", _AUDIO_C02 / "Between Two Chapters.wav"),
    TrackSpec(3,  "Tucked in the Drawer", _AUDIO_C01 / "Tucked in the Drawer.wav", _AUDIO_C02 / "Tucked in the Drawer.wav"),
    TrackSpec(4,  "Drifting Like Dust",   _AUDIO_C01 / "Drifting Like Dust.wav",   _AUDIO_C02 / "Drifting Like Dust.wav"),
    TrackSpec(5,  "Catching Orange Light",_AUDIO_C01 / "Catching Orange Light.wav",_AUDIO_C02 / "Catching Orange Light.wav"),
    TrackSpec(6,  "Steady Feelings",      _AUDIO_C01 / "Steady Feelings.wav",      _AUDIO_C02 / "Steady Feelings.wav"),
    TrackSpec(7,  "A Rhythm at Dusk",     _AUDIO_C01 / "A Rhythm at Dusk.wav",     _AUDIO_C02 / "A Rhythm at Dusk.wav"),
    TrackSpec(8,  "No Need to Rush",      _AUDIO_C01 / "No Need to Rush.wav",      _AUDIO_C02 / "No Need to Rush.wav"),
    TrackSpec(9,  "Quiet Shape at Night", _AUDIO_C01 / "Quiet Shape at Night.wav", _AUDIO_C02 / "Quiet Shape at Night.wav"),
    TrackSpec(10, "Pocket of Shy Courage",_AUDIO_C01 / "Pocket of Shy Courage.wav",None),
    TrackSpec(11, "Mist on the Window",   _AUDIO_C01 / "Mist on the Window.wav",   _AUDIO_C02 / "Mist on the Window.wav"),
    TrackSpec(12, "Late and Low Lamp",    _AUDIO_C01 / "Late and Low Lamp.wav",    _AUDIO_C02 / "Late and Low Lamp.wav"),
    TrackSpec(13, "Last Bookstore Hour",  _AUDIO_C01 / "Last Bookstore Hour.wav",  _AUDIO_C02 / "Last Bookstore Hour.wav"),
)
...
def ensure_inputs_exist() -> None:
    ...
    if missing:
        raise FileNotFoundError("missing required S01E04 input(s): " + ", ".join(missing))
...
def build_music_note_overlay(duration: float, temp_root: Path, *, start_seconds: float = 0.0) -> tuple[Path, dict[str, object]]:
    output = temp_root / "s01e04-render-01-music-note-overlay.mov"
    if output.exists() and output.stat().st_size > 0:
        return output, {
            ...
            "note_count": 3,
            "motion": "EP1 render-05 tiny warm vector notes adapted to the S01E04 header zone",
...
def build_header_overlay_concat(timeline: list[TimelineTrack], duration: float, temp_root: Path) -> tuple[Path, dict[str, object]]:
    ...
    concat_path = temp_root / "s01e04-header-overlay.ffconcat"
...
def build_combined_overlay_concat(timeline: list[TimelineTrack], cues: list[Cue], duration: float, temp_root: Path) -> tuple[Path, dict[str, object]]:
    ...
    frame_dir.mkdir(parents=True, exist_ok=True)
    concat_path = temp_root / "s01e04-combined-overlay.ffconcat"
...
def build_equalizer_overlay(audio_path: Path, duration: float, temp_root: Path, *, start_seconds: float = 0.0) -> Path:
    output = temp_root / "s01e04-render-01-ribbon-equalizer-overlay.mov"
    if output.exists() and output.stat().st_size > 0:
        return output
...
def build_visual_assets(audio_path: Path, duration: float, temp_root: Path, *, start_seconds: float = 0.0) -> dict[str, Path]:
    particle_path = temp_root / "s01e04-render-01-particles.png"
    star_path = temp_root / "s01e04-render-01-star-light.png"
    lamp_path = temp_root / "s01e04-render-01-lamp-glow.png"
    light_path = temp_root / "s01e04-render-01-light-sweep.png"
    glow_path = temp_root / "s01e04-render-01-night-glow.png"
    reflection_path = temp_root / "s01e04-render-01-reflection.png"
    shadow_path = temp_root / "s01e04-render-01-shadow.png"
...
def build_subtitle_overlay(
    ...
    *,
    ...
    output = temp_root / f"s01e04-subtitle-overlay-{overlay_signature}.mov"
    if output.exists() and output.stat().st_size > 0:
...
def render_video(
    ...
    *,
    ...
    temp_video = temp_root / f"s01e04-render-01-{temp_suffix}.tmp.mp4"
...
def run_ffmpeg_segment_render(
    ...
    if video_out.exists():
        video_out.unlink()
    prepare_output(video_out)
    temp_video = temp_root / f"s01e04-render-01-segment-{segment.index:02d}.tmp.mp4"
    if temp_video.exists():
        temp_video.unlink()
...
def assemble_chunked_video(
    ...
    prepare_output(video_out)
    concat_path = temp_root / "s01e04-render-01-segments.ffconcat"
    write_concat_file(segment_paths, concat_path)
    temp_video = temp_root / "s01e04-render-01-final.tmp.mp4"
    if temp_video.exists():
        temp_video.unlink()
    command = [
        "ffmpeg",
...
def extract_snapshots(video_path: Path, output_root: Path, duration: float) -> list[str]:
    specs = [
        ("s01e04-render-01-sample-01-open.png", 2.0),
        ("s01e04-render-01-sample-02-track03.png", 370.0),
        ("s01e04-render-01-sample-03-track06.png", 900.0),
        ("s01e04-render-01-sample-04-track09.png", 1450.0),
        ("s01e04-render-01-sample-05-track12.png", 2090.0),
        ("s01e04-render-01-sample-06-ending.png", max(1.0, duration - 8.0)),
    ]
...
def main() -> int:
    ...
    summary = {
        ...
        "decode_check": decode,
        "snapshots": snapshots,
        "visual_policy": "EP1 render-05 headphone icon, music notes, particles/light, custom ribbon-dot equalizer, and near-still motion are the canonical video overlay standard; S01E04 adapts them to the bookstore afternoon image",
