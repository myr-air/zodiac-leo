# S01E04 YouTube API Thumbnail Upload Package — Bookstore Afternoon Longplay

Status: thumbnail upload completed / video ID OMjvEEAIFSU / public release blocked
Episode: `s01e04-bookstore-afternoon-longplay`
Prepared: 2026-06-05

## 0. Boundary

This file records the narrow thumbnail upload package for S01E04 after the local thumbnail derivative was created from the accepted `vis-c01` visual background. It scopes only one YouTube Data API `thumbnails.set` call for existing S01E04 video ID `OMjvEEAIFSU`, after authenticated channel verification succeeds and after OAuth client secrets/token-cache paths are supplied outside this repo.
...
thumbnails.set -> upload the selected local PNG for S01E04 video ID OMjvEEAIFSU
```
...
|---|---|---|
| Thumbnail candidate | `candidates/s01e04-bookstore-afternoon-longplay/thumbnail/s01e04-bookstore-afternoon-longplay.thumbnail-layered-premium-1280x720.png` | User-approved local PNG derivative from the accepted `vis-c01` background; `1280x720` dimensions. |
...
| Video ID | `OMjvEEAIFSU` | Existing S01E04 YouTube video ID from the guarded video upload. Do not store private account state here. |
...
uv run --project /Users/xiivth/workspaces/zodiac/leo --with google-api-python-client --with google-auth --with google-auth-oauthlib python scripts/youtube_api_thumbnail.py \
  ...
  --thumbnail candidates/s01e04-bookstore-afternoon-longplay/thumbnail/s01e04-bookstore-afternoon-longplay.thumbnail-layered-premium-1280x720.png \
  --thumbnail-source channel/episodes/s01e04-bookstore-afternoon-longplay/source/youtube-api-thumbnail-upload-package.md
```
...
```text
Scope: one S01E04 thumbnails.set upload for the selected local PNG after an existing YouTube video ID exists
...
Scope: thumbnail-only YouTube Data API thumbnails.set execution after channel verification and after S01E04 video ID exists
Current local thumbnail: selected PNG derivative from vis-c01 under candidates/s01e04-bookstore-afternoon-longplay/thumbnail/
Execution result: thumbnails.set completed for video ID OMjvEEAIFSU on verified channel UC4qQwe3oiykEGhL_WyVFtMg and returned youtube#thumbnailSetResponse
