...
A late-afternoon college classroom by a tall window: warm sun rectangles across desks, open notebook and graph paper, one folded quiz corner, yellow highlighter, pencil case, soft backpack presence, and enough calm negative space for a cozy longplay title and later subtitles. The image should feel like curiosity turning into quiet familiarity before class ends.
...
```text
Use-case: original reference-free 16:9 key visual for a cozy vocal longplay cover and later local layout planning.
Canvas/aspect: widescreen 16:9 with clean negative space in the upper-left and a simple lower band for subtitles.
Scene: late-afternoon college classroom beside a tall sunlit window, soft campus trees outside, warm light rectangles falling across desks.
...
| Element | Reserved area | Source check |
|---|---|---|
| Episode title | upper-left with about 10-12 percent top margin | keep free of face, charm, and bright window edge |
| Track / Now Playing block | top-left below title or compact stacked header | keep secondary and lightweight |
| Subtitles | lower-left to middle-left clear lane | background must stay simple enough for readable lyric overlays |
| Equalizer | small bottom-right | low contrast, secondary to desk/window scene |
| Focal subject | center-right 60 percent | do not cover notebook, backpack charm, or sunlight rectangle with text |
| Mobile crop | protect charm, desk objects, and title area from outer 10 percent | image should still read in thumbnail crop |

## Source Review Need Before Any Candidate Gate

- Confirm the scene stays original and reference-free.
- Confirm character framing remains adult/college-coded, wholesome, non-childlike, and non-sexualized.
- Confirm overlays remain secondary to the classroom-window image.
- Confirm no provider/image-generation action is implied by this source file.
