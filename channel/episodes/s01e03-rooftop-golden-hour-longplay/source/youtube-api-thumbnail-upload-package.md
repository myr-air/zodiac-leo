# S01E03 YouTube API Thumbnail Upload Package — Rooftop Golden Hour Longplay

Status: thumbnail-only follow-up completed after explicit user gate
Episode: `s01e03-rooftop-golden-hour-longplay`
Prepared: 2026-06-03

## 0. Boundary

This file records the narrow thumbnail follow-up for S01E03 after the local thumbnail derivative was created from the accepted `vis-c01` visual background. It scopes only one YouTube Data API `thumbnails.set` call for existing S01E03 video ID `2P6fPs7NB0E`, after authenticated channel verification succeeds and after OAuth client secrets/token-cache paths are supplied outside this repo.
...
thumbnails.set -> upload the selected local PNG for S01E03 video ID 2P6fPs7NB0E
```

The video upload remains handled by `source/youtube-api-video-upload-package.md` and `scripts/youtube_api_video_upload.py`. The thumbnail helper must not call `videos.insert`.
...
|---|---|---|
| Thumbnail candidate | `candidates/s01e03-rooftop-golden-hour-longplay/thumbnail/s01e03-rooftop-golden-hour-longplay.thumbnail-layered-premium-1280x720.png` | User-approved local PNG derivative from the accepted `vis-c01` background; `1280x720` dimensions. |
...
| Video ID | `2P6fPs7NB0E` | Existing S01E03 YouTube video ID from the guarded video upload. Do not store private account state here. |
...
uv run --project /Users/xiivth/workspaces/zodiac/leo --with google-api-python-client --with google-auth --with google-auth-oauthlib python scripts/youtube_api_thumbnail.py \
  ...
  --thumbnail candidates/s01e03-rooftop-golden-hour-longplay/thumbnail/s01e03-rooftop-golden-hour-longplay.thumbnail-layered-premium-1280x720.png \
  --thumbnail-source channel/episodes/s01e03-rooftop-golden-hour-longplay/source/youtube-api-thumbnail-upload-package.md
```

Execution mode under this gate:
...
```text
Scope: one S01E03 thumbnails.set upload for the selected local PNG after an existing YouTube video ID exists
...
Scope: thumbnail-only YouTube Data API thumbnails.set execution after channel verification and after S01E03 video ID exists
Current local thumbnail: selected PNG derivative from vis-c01 under candidates/s01e03-rooftop-golden-hour-longplay/thumbnail/
Execution result: thumbnails.set completed for video ID 2P6fPs7NB0E on verified channel UC4qQwe3oiykEGhL_WyVFtMg and returned youtube#thumbnailSetResponse with a maxres 1280x720 variant
