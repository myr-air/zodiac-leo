...
| Overlay priority | pass_source | Title, subtitles, and equalizer are kept secondary in source planning. |

## Verdict

`pass_internal_source_draft`

This verdict only means the visual source direction is coherent enough for continued source planning. It is not image generation approval, candidate acceptance, render/export approval, upload approval, or rights/platform assurance.

## Re-Review Triggers

Re-review if the prompt, subject, style, source image use, overlay plan, crop, candidate file, generation method, provider/account path, or release intent changes.
