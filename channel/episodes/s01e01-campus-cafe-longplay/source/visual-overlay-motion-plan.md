...
- Subtitle source: Gate 10 draft timings are human watch-passed source-only; final subtitle sidecars still require a separate assembly-package gate.
...
For any future assembly package, preserve the V6 visual language but enforce a conservative crop-safe carry-forward rule: keep critical header/title text within a target-safe inset unless a fresh proof shows the target output is safe. Use the proof sheets to check the top-left header, middle-left subtitles, and lower-right equalizer before final sidecar promotion or render/export planning.
