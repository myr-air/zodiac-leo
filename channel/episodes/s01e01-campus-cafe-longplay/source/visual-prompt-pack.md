...
This file packages reference-free visual prompts for future manual visual planning only. It does not approve image generation, image editing, reference-image input use, screenshots, candidate media, render/export, upload, publishing, provider/account/API/browser automation, or positive rights/platform claims.

Do not use the local signature reference images as provider inputs or exact-copy targets without a separate future gate. Do not record visual candidate facts until real user-supplied local image files exist.
...
- Keep title overlay in the upper-left or upper-center with at least 12 percent top margin.
- Keep the lower 18 percent visually simple for future subtitles.
- Keep the face, notebook, coffee, and crescent-vinyl charm away from overlay text.
- Keep key objects away from the outer 10 percent for mobile crop safety.
- Equalizer or chapter cue, if later used, must stay small and secondary.

## 6. Immediate Visual Reject Criteria

Reject or regenerate if a future candidate shows:
...
Re-review this file if the prompt, subject, style, source image use, provider/model, crop, overlay plan, candidate file, render/export intent, or upload/public intent changes.
